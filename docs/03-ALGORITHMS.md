# 03. Algorithms — 분산 SD 알고리즘 비교와 선택 근거

[02-SCENARIOS](./02-SCENARIOS.md) 에서 도출된 요구사항(분포 보존, RTT 상각, 다중 클라이언트 격리, 장애 내성)을 만족시키는 분산 Speculative Decoding 알고리즘을 비교하고, 본 프로토타입이 Client-Server 변형을 선택한 근거를 정리한다.

---

## 1. Speculative Decoding 기본 복기
...
**수학적 보장**: 이 과정의 전체 출력 분포는 $M_p$ 단독 샘플링과 **정확히 동일**하다 ([Chen et al., 2023](https://arxiv.org/abs/2302.01318) Theorem 1, [Leviathan et al., 2023](https://arxiv.org/abs/2211.17192) §2.2). 즉 품질은 보존하고 latency 만 개선한다.
...
### 2.1 Monolithic (vLLM in-process)
...
**단점**
- Client 가 target 모델을 로컬에 로드해야 함 → [02 Scenario 1](./02-SCENARIOS.md#scenario-1--엣지-클라이언트--클라우드-target) 성립 불가.
...
### 2.2 Client-Server (본 프로토타입)

Draft 는 client 에서, target verify 는 server 에서. 둘은 ZMQ 메시지로 통신.

**장점**
- [02 Scenario 1, 2](./02-SCENARIOS.md) 를 자연스럽게 만족.
...
**왕복 오버헤드 완화**: 한 왕복에 $K$ 토큰을 담아 상각 ([01-PROBLEM](./01-PROBLEM.md) § 정량적 Impact).
...
## 4. Rejection Sampling 의 핵심 수식 (구현에 직결)

[06-VERIFICATION](./06-VERIFICATION.md) 에서 자세히 다루지만, 본 문서에서는 "왜 이 알고리즘이 분포를 보존하는가" 만 요약한다.
...
**다음 섹션**: 선택한 Client-Server 아키텍처를 어떻게 구현했는지, 프로토콜과 컴포넌트 관점에서 기술한다 → [04-DESIGN](./04-DESIGN.md)
