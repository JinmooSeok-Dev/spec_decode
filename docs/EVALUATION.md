# Evaluation — 정확성 검증과 성능 측정

[VERIFICATION](./VERIFICATION.md) 의 rejection sampling 이 수학적으로 분포를 보존한다는 보장은 구현 버그로 깨질 수 있다. 본 문서는 구현의 정확성과 성능을 검증하는 실험 계획과 결과를 기록한다.
...
## Adaptive K 컨트롤러 단위 테스트

### 목표

합성 $(\text{RTT}, \alpha)$ 궤적을 주입해 [ADAPTIVE_CONTROL](./ADAPTIVE_CONTROL.md) 의 $K^*$ 공식이 의도대로 수렴하는지 확인.
...
## vLLM 백엔드 실측 (Phase 2 S1)

`VllmVerifier` 가 실제 엔진 위에서 동작하는지를 in-process smoke 로 검증한 결과입니다.

### 환경

| 항목 | 값 |
|---|---|
| GPU | NVIDIA RTX 3060 (sm_86, 12 GB) |
| vLLM | 0.19.1 (V1 engine) |
| Target | `gpt2` (max_position_embeddings=1024, fp16→bf16) |
| Proposer | N-gram, `num_speculative_tokens=3`, adaptive K off |
| Prompt | `"a b a b a b a b"` (N-gram 이 뽑기 좋은 반복 패턴) |
| `gpu_memory_utilization` | 0.5, `enforce_eager=True` |

### 실측 결과 — 단일 클라이언트, `max_tokens=20`

| req | draft | accepted | bonus | latency (ms) |
|---|---|---|---|---|
| 1 | 3 | 0 | yes | 9.5 (warmup 제외) |
| 2 | 3 | 3 | yes | 4.5 |
| 3 | 3 | 3 | yes | 4.3 |
| 4 | 3 | 3 | yes | 4.6 |
| 5 | 3 | 3 | yes | 4.6 |

- **Total**: 9 requests, 21 drafts, 14 accepted, 9 bonus
- **α (acceptance rate)**: 66.7% (반복 패턴 유리)
- **Wall clock**: 첫 요청 부터 완료까지 ~50 ms (warmup 후 정상 상태)
- **생성 토큰 수**: 20 정확 (`--max-tokens=20` 준수)

### 관찰 포인트

- vLLM 의 **prefix caching** 이 활성화되어 있어 첫 요청은 200 ms 수준(prompt prefill) 이후 5 ms 수준으로 수렴. 같은 클라이언트의 연속 요청에서 context prefix 가 재사용됨.
- HfVerifier (Phase 1) 에서 동일 워크로드를 매번 full forward 로 돌리면 요청당 수십–수백 ms 가 드는 것과 대비. 즉 Phase 2 전환의 실질적 이득이 확인됨.
- **greedy only** 제약 확인: `temperature > 0` 요청은 `NotImplementedError`. 소스 레퍼런스 [VERIFICATION § Backend 별 구현 차이](./VERIFICATION.md#backend-별-구현-차이).

### 재현 방법

```bash
# 별개 터미널에서 서버
distspec-server --backend vllm --model gpt2 \
    --listen-address 127.0.0.1:8000 --gpu-memory-utilization 0.5

# 클라이언트
distspec-client --prompt "a b a b a b a b" --server 127.0.0.1:8000 \
    --tokenizer gpt2 --temperature 0 --max-tokens 20 \
    --draft-method ngram
```

서버 터미널의 INFO 로그에 `verify client=... draft=N accepted=M bonus=yes latency=X.Xms` 가 요청마다 출력되며, Ctrl+C 종료 시 `Server Metrics` 블록으로 집계 확인 가능.

---

## 풀 성능 벤치 (Phase 2 이후 — 계획)
...
현재 레이아웃 기준 — `pip install -e ".[torch,dev]"` (또는 `.[vllm,dev]`) 후:

```bash
# CI-safe 전체 (수 초, GPU 불필요):
pytest -m "not slow and not vllm"

# HuggingFace 소형 모델 end-to-end (GPU 권장, ~1-2분):
pytest -m slow

# vLLM 백엔드 end-to-end (CUDA GPU 필수, 수 분):
pytest -m vllm

# CPU 마이크로벤치 3종 (CI 용):
python benchmarks/bench_ngram.py --json
python benchmarks/bench_protocol.py --json
python benchmarks/bench_sampling.py --json
```

---

**다음 섹션**: Phase 1 에서 의도적으로 다루지 않은 항목들과 Phase 2 (vLLM 통합) 계획 → [ROADMAP](./ROADMAP.md)
