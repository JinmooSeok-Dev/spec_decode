# 04. System Design — Client-Server 아키텍처

[03-ALGORITHMS](./03-ALGORITHMS.md) 에서 선택한 Client-Server 변형을 어떻게 구현했는지, 컴포넌트 / 메시지 / 상태 관점에서 정리한다. 각 설계 결정은 [02-SCENARIOS](./02-SCENARIOS.md) 의 특정 시나리오/요구에서 도출됐다.
...
- 타임아웃 시 `zmq.error.Again` 예외 → `DraftClient.generate()` 에서 재시도, 연속 실패는 [07-ADAPTIVE_CONTROL](./07-ADAPTIVE_CONTROL.md) 의 FSM 으로 전파.
...
**다음 섹션들**:
- Draft 를 어떻게 만드는지: [05-DRAFT_METHODS](./05-DRAFT_METHODS.md)
- Target 검증과 분포 보존: [06-VERIFICATION](./06-VERIFICATION.md)
- Adaptive K 와 Fault-Tolerant FSM: [07-ADAPTIVE_CONTROL](./07-ADAPTIVE_CONTROL.md)
